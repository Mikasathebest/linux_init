...
if ($OSTYPE != "Win32") then
  # WA - remarked to leave the path from the previous process (CadenceShell, etc.) set path = ""
else
  if ($?SYSPATH) then
    setenv PATH "$SYSPATH"
  else
...
if ($OSTYPE == "Linux" || $OSTYPE == "SunOS") then
  ...
  # remarked since not working on WAP: limit coredumpsize 1
endif
...
if ($OSTYPE == "Linux" || $OSTYPE == "SunOS") then
  # replaced with following lines: source ${HOME}/.cshrc_tools
  if (-f /home/ip/shared/inf/rbs/1.0/utils_latest/rbs_src/UserEnv/nvglobal_tools.cshrc) then
    source /home/ip/shared/inf/rbs/1.0/utils_latest/rbs_src/UserEnv/nvglobal_tools.cshrc
  endif
endif
...
##echo "trace_t10"
# added protection since .login does not exist in ILYK
...
# commented out because it is sourced at end of common.cshrc
